...
-- MAGIC To run this demo, just select the cluster `dbdemos-lakehouse-iot-platform-kevin_barlow` from the dropdown menu ([open cluster configuration](https://e2-demo-field-eng.cloud.databricks.com/#setting/clusters/0213-234830-wrke7ap3/configuration)). <br />
...
GRANT SELECT ON TABLE kbarlow.iot.turbine TO `analysts`;
GRANT SELECT ON TABLE kbarlow.iot.sensor_bronze TO `analysts`;
GRANT SELECT ON TABLE kbarlow.iot.sensor_hourly TO `analysts`;
GRANT SELECT ON TABLE kbarlow.iot.historical_turbine_status TO `analysts`;

-- We'll grant an extra MODIFY to our Data Engineer
GRANT SELECT, MODIFY ON SCHEMA kbarlow.iot TO `dataengineers`;
