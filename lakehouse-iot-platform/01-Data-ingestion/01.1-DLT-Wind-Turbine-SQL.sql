...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=1444828305810485&notebook=%2F01-Data-ingestion%2F01.1-DLT-Wind-Turbine-SQL&demo_name=lakehouse-iot-platform&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-iot-platform%2F01-Data-ingestion%2F01.1-DLT-Wind-Turbine-SQL&version=1">
...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/e0b874a7-3edb-4972-b87c-61378aa84d2a" target="_blank">IOT Wind Turbine Delta Live Table pipeline</a> to see it in action.<br/>
-- MAGIC
...
-- %python
-- display(spark.read.json('/Volumes/kbarlow/iot/turbine_raw_landing/turbine'))
-- display(spark.read.json('/Volumes/kbarlow/iot/turbine_raw_landing/historical_turbine_status')) #Historical turbine status analyzed

-- COMMAND ----------

-- DBTITLE 1,Wind Turbine sensor data
-- %python
-- display(spark.read.parquet('/Volumes/kbarlow/iot/turbine_raw_landing/incoming_data'))
...
CREATE OR REPLACE STREAMING TABLE turbine (
  CONSTRAINT correct_schema EXPECT (_rescued_data IS NULL)
)
COMMENT "Turbine details, with location, wind turbine model type etc"
AS SELECT * FROM cloud_files("/Volumes/kbarlow/iot/turbine_raw_landing/turbine", "json", map("cloudFiles.inferColumnTypes" , "true"))

-- COMMAND ----------

-- DBTITLE 1,Wind Turbine sensor 
CREATE OR REPLACE STREAMING TABLE sensor_bronze (
  CONSTRAINT correct_schema EXPECT (_rescued_data IS NULL),
...
AS SELECT * FROM cloud_files("/Volumes/kbarlow/iot/turbine_raw_landing/incoming_data", "parquet", map("cloudFiles.inferColumnTypes" , "true"))

-- COMMAND ----------

-- DBTITLE 1,Historical status
CREATE OR REPLACE STREAMING TABLE historical_turbine_status (
  CONSTRAINT correct_schema EXPECT (_rescued_data IS NULL)
)
...
AS SELECT * FROM cloud_files("/Volumes/kbarlow/iot/turbine_raw_landing/historical_turbine_status", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
CREATE OR REPLACE LIVE TABLE sensor_hourly (
...
CREATE OR REPLACE LIVE TABLE turbine_training_dataset 
COMMENT "Hourly sensor stats, used to describe signal and detect anomalies"
AS
...
-- MAGIC
-- MAGIC ## Conclusion
-- MAGIC Our <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/e0b874a7-3edb-4972-b87c-61378aa84d2a" target="_blank">DLT Data Pipeline</a> is now ready using purely SQL. We have an end 2 end cycle, and our ML model has been integrated seamlessly by our Data Engineering team.
-- MAGIC
-- MAGIC
...
-- MAGIC
-- MAGIC We are now ready to build our <a href="/sql/dashboards/03051bc8-6bba-4f40-a198-e385930cd337">DBSQL Dashboard</a>to track the main KPIs and status of our entire Wind Turbine Farm and build complete <a href="/sql/dashboards/90b2c9c7-e5c7-46d8-b7fe-5b98ed84d5d1">Predictive maintenance DBSQL Dashboard</a>.
-- MAGIC
-- MAGIC
