...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE kbarlow.c360.ml_churn_features AS SELECT * FROM kbarlow.c360.churn_features;
...
spark.createDataFrame(expectations, headers.split(",")) \
  ...
  .write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("kbarlow.c360.dlt_expectations")

# COMMAND ----------

spark.sql(f'alter table kbarlow.c360.dlt_expectations set tblproperties (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)')
