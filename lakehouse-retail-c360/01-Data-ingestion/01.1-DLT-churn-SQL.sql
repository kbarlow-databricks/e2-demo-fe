...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-churn" href="#joblist/pipelines/c2aff205-38cd-43ff-9745-5cee8313f85c" target="_blank">Churn Delta Live Table pipeline</a> to see it in action.<br/>
...
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/kbarlow/c360/c360/users'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming orders (json)
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/kbarlow/c360/c360/orders'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming clickstream (csv)
-- MAGIC %python
-- MAGIC display(spark.read.csv('/Volumes/kbarlow/c360/c360/events', header=True))
...
AS SELECT * FROM cloud_files("/Volumes/kbarlow/c360/c360/events", "csv", map("cloudFiles.inferColumnTypes", "true"))
...
AS SELECT * FROM cloud_files("/Volumes/kbarlow/c360/c360/orders", "json")
...
AS SELECT * FROM cloud_files("/Volumes/kbarlow/c360/c360/users", "json", map("cloudFiles.inferColumnTypes", "true"))
...
-- MAGIC #                                                                 Model name         |        
-- MAGIC #                                                                     |              |        
-- MAGIC predict_churn_udf = mlflow.pyfunc.spark_udf(spark, "models:/kbarlow.c360.dbdemos_customer_churn@prod", "int")
...
-- MAGIC
-- MAGIC Open the <a dbdemos-pipeline-id="dlt-churn" href="#joblist/pipelines/c2aff205-38cd-43ff-9745-5cee8313f85c" target="_blank">Churn Delta Live Table pipeline</a> and click on start to visualize your lineage and consume the new data incrementally!
